/*jeu.c
Par Basilou, définit la fonction jouer()*/
#include<stdlib.h>
#include<stdio.h>
#include<SDL.h>
#include<SDL_image.h>
...
//une surface pour chaque stripe
    ...
    switch(direction)
{
    ...
    case BAS:
        if (pos->y +1>11) // Si le joueur dépasse l'écran, on arrête
            break;
        if (carte[pos->x][pos->y +1] == MUR) // S'il y a un mur, on arrête
            break;
        // Si on veut pousser une caisse, il faut vérifier qu'il n'y a pas de mur derrière (ou une autre caisse, ou la limite du monde)
        if ((carte[pos->x][pos->y +1] == CAISSE || carte[pos->x][pos->y +1] == CAISSE_OK) &&
            (pos->y +2 >11) || carte[pos->x][pos->y +2] == MUR ||
            carte[pos->x][pos->y +2] == CAISSE || carte[pos->x][pos->y +2] == CAISSE_OK)
            break;
    ...
    case DROITE:
        if (pos->x +1 < 11) // Si le joueur dépasse l'écran, on arrête
            break;
        if (carte[pos->y][pos->x +1] == MUR) // S'il y a un mur, on arrête
            break;
        // Si on veut pousser une caisse, il faut vérifier qu'il n'y a pas de mur derrière (ou une autre caisse, ou la limite du monde)
        if ((carte[pos->y][pos->x +1] == CAISSE || carte[pos->y][pos->x +1] == CAISSE_OK) &&
            (pos->x +2 >11) || carte[pos->y][pos->x +2] == MUR ||
